...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "bg-surface-overlay text-text-primary flex h-full w-full flex-col overflow-hidden rounded-md",
        className
      )}
      {...props}
    />
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:bg-state-hover data-[selected=true]:text-text-primary [&_svg:not([class*='text-'])]:text-text-muted relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
